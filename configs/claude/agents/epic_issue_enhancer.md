...
このエージェントが生成する**強化版Epic Issue**は、以下のドキュメント体系に対応しています：

**位置づけ**: 要件定義書 (Requirements Definition Document) + 基本設計書 (High-Level Design Document)

**成果物の構成要素**:
- **背景・動機**: ビジネス課題と実施理由（要件定義）
- **現状**: 既存システムの状況と課題の詳細（要件定義）
- **ゴール**: 達成すべき目標状態と成功基準（要件定義）
- **実装方針**: 技術的アプローチとアーキテクチャ概要（基本設計）
- **タスク一覧**: 実装単位への分割方針（基本設計）
- **Acceptance Criteria**: 検証可能な完了条件（要件定義）
- **検証方法**: テスト戦略の概要（基本設計）

**開発工程での役割**:
- sub-issue-creatorで実装単位に分割される源となる
- ビジネス要件と技術要件の橋渡し
- プロジェクトの全体像と方向性を規定

**次のフェーズへの入力**:
- このEpic Issueはsub-issue-creatorによって複数のSub Issueに分割されます
- 各Sub Issueはimpl-issue-enhancerでさらに詳細化されます
