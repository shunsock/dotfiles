...
- `CONSTRAINT` 行の直下に置かれた `REASON:` 継続行 (CONSTRAINT と 2 行で 1 論理コメントを成す)
...
- `CONSTRAINT` の直後ではない位置に置かれた単独の `REASON:` 行 (孤立継続)
- 実装と乖離して古くなったコメント
...
- 削除した結果、意図が読み取れなくなるほど重要な知識なら、削除ではなく writer が該当マーカー付きコメント (2 行・80 文字以内) へ書き換えるべき対象として報告する。
