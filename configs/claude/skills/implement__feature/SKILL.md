...
description: >-
  ...
  テストリストを立案し、Phase 2 で opus モデル固定のサブエージェント
...
実装の進め方 (計画の粒度・テストファーストの徹底度・レビューの反復回数) はスキルに宣言しない限りセッションごとにばらつく。
このスキルは実装プロセス自体を宣言し、実装を opus 固定のサブエージェントへ、レビューを観点別スキルへ委譲することで、同じ品質プロセスを再現可能にする。
レビューには計測可能な pass 基準と反復上限 (3 回) があり、指摘の放置と無限の手直しの両方を防ぐ。
...
2. タスクを **small テストで検証できる最小単位** を目標に分割する。
   small テストで検証できない残余のみ、medium → large の順でテストサイズを上げた作業単位とする
...
- プロンプトには作業単位の範囲・テストリスト・変更対象ファイル・完了条件を明記する
  (エージェント側の「入力の契約」を満たす)
- 依存関係のない作業単位は並行起動してよい。依存のある作業単位は前段の完了報告を確認してから起動する
- 各エージェントの TDD Implementation Report を確認し、未完了・保留が報告された場合は
  範囲を調整して再委譲するか、理由を最終サマリーに記録する
