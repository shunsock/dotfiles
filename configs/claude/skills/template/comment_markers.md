...
- **ペア形式必須**: `CONSTRAINT` は必ず次の 2 行ペアで書く。1 行目が満たすべき制約
  ...
  1 論理コメント (2 行) であり、通常の 2 行・80 文字制約にもそのまま従う。
...
- **`CONSTRAINT` はペア形式必須・1 ファイル 3 件まで** (上記「CONSTRAINT の特則」参照)。
...
- **writer (`write__structured_comment`) の出力集合** = 上記 7 マーカーちょうど。
  これ以外のコメントを生成しない。すべて 2 行・80 文字・issue/PR 番号禁止に従い、
  `CONSTRAINT` は必ず `REASON` 付きペア・1 ファイル 3 件以内で書く。
