...
Issue 番号がどの情報源からも検出できない場合は、説明文の生成を中断する。そしてユーザーに関連 Issue の作成を案内する。`Closes` 行の無い説明文を出力してはならない。
...
| セクション | 書く内容 |
|---|---|
| 概要 | 何をしたか・何を解決するかを 1〜2 文の箇条書きで。1 項目 1 文とし句点で改行する。`Closes #<issue-number>` を必ず添える (必須。関連 Issue が無ければユーザーに作成を案内する) |
| 背景 | 変更のきっかけとなった状況を箇条書きで。1 項目 1 文とし句点で改行する |
...
生成した説明文を、共有評価基準の評価観点でセルフレビューする。
基準ファイルは `~/.claude/skills/template/evaluation_pull_request.md` である。
評価観点の内容は同ファイルが single source of truth として所有する。

- いずれかの観点を満たさない場合は修正してから出力する
- 参考文献は全リンクのリンク切れ (404・到達不能) と PR との関連を確認する。無関係なものは削除する

このフェーズは生成者自身によるセルフチェックである。
独立した評価は、呼び出し元 (`submit__pull_request` / `restart__pull_request`) が同じ基準ファイルで `skeptical-reviewer` を起動して行う。
