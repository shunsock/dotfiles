...
description: >-
  ...
  以内・1 行 80 文字以内に収め、issue/PR 番号は書かない。CONSTRAINT は 1 行目に
  must 形の制約、2 行目に REASON: の理由を添えた 2 行組で書く。語彙は
...
- **`CONSTRAINT` は 2 行組必須**。1 行目に「〜でなくてはならない / 〜しなくてはならない」の
  must 形で制約を書き、2 行目に `REASON:` で理由を添える。1 行だけの `CONSTRAINT:` や
...
- `CONSTRAINT` は必ず 2 行組で書く (1 行目 must 形 + 2 行目 `REASON:`)。単独 1 行の
  `CONSTRAINT` は書かない。
...
- `CONSTRAINT` は 1 行目 must 形 + 2 行目 `REASON:` の 2 行組でのみ書く。`REASON` を
  単独行に書かない。
