...
あなたは日本語テクニカルライティングのレビューの専門家である。日本語 Markdown を変更したら、textlint の日本語プリセットを通す。さらに、文の途中で折り返した意味のない改行 (文中ハードラップ) を検出する。検出した段落や箇条書き項目は 1 行に連結する。また、文章中で言及するコードスニペットは、その参照元へリンクするとよい。
...
- 指摘行ごとにファイルを読み、ルールを満たすよう文章を直す。
  - `sentence-length` / `max-ten`:
    - 一文を 2 文へ書き分ける (各文が句点で終わる形にする)。
    - 分割した 2 文は、各文を 1 項目とする箇条書きへ変換する。
    - 箇条書き化が文脈上不自然な場合は、語句を削る・言い換えて短くする。
    - 文の途中で改行し、行を短く見せる対処は禁止する。
    - 改行は文の区切りにならず、`sentence-length` も解消しない。むしろ句点以外で文が割れる原因になる。
...
- プローズ段落の中で、句点 (または句点を想定できる文末) でない位置で折り返した改行を指す。これを文中ハードラップと呼ぶ。
- 文末記号は `。` `！` `？` (全角・半角の両方) を含む。`。」` `。）` のように閉じ括弧を伴う文末も文末として扱う。
- 句点が省略されていても、意味的に文末となる位置は文末として扱う。この位置の改行は連結せず、必要なら句点を補って残す。
- 読点・文節の途中など、文が継続している位置の改行のみを連結対象とする。
...
**検出の補助:** 候補行を機械的に絞り込むには次のワンライナーが使える。文末記号と継続記号のどちらでも終わらない、文字を含む行をおおまかな候補として挙げる。
...
このワンライナーはあくまで候補を挙げるだけである。日本語の文末判定は難しく、見出し・表・コードフェンス・リスト項目を誤って拾う。そのため、最終判断は必ず Read で各候補行とその次行を確認してから行う。自前の大掛かりなパーサは作らない。

**調整方法:** 文中で割れている行を次の行と連結する。

- 日本語の連結時はスペースを入れない。連結境界の両側がどちらも ASCII の場合のみ半角スペース 1 個を保持する。
- 連結はコードの挙動・文意を変えてはならない。
...
- Phase 3〜4 の編集をすべて終えてから、各プローズ段落に残った改行を 1 つずつ調べる。
- 改行の直前が句点 (または句点を想定できる文末) で終わっていなければ、文中で割れた改行とみなす。次行と連結して 1 行へ戻す。
- 連結後に Phase 2 の textlint を再実行し、新たな指摘が出ないことを確認する。
- 判断に迷う改行は連結せず Phase 7 の報告に残す。
...
- 実在し位置を特定できるシンボルを指すものだけリンクする。
- 説明用や擬似コードはそのままにする。
- ユーザーが望まなければ、このフェーズは丸ごと省略する。
...
**文の積み上げの検出 (箇条書き化の候補):** 1 行に句点を 3 個以上含む行を候補として挙げる。

```bash
# 句点が 3 個以上ある行を箇条書き化の候補として列挙する。
grep -nE '。.*。.*。' <file>
```

- 候補行を Read で確認する。コードフェンス・表・引用の行は除外する。
- 列挙・定義・手順を連ねた行は、Phase 3 の方針どおり箇条書きへ変換する。
- 地の文として自然に流れる段落は変換せず残してよい。
- 変換を見送った候補は、理由とともに Phase 7 の報告へ記載する。

**自己言及チェック:** 対象ファイルがルールを記述する文書 (このスキル定義を含む) か確認する。該当する場合、変更後の文書が自らの定めるルールに違反していないか点検する。違反があれば同じ手順で修正する。
...
### 文の積み上げの調整
- 箇条書きへ変換した箇所: (一覧、無ければ none)
- 変換を見送った箇所と理由: (一覧、無ければ none)
...
textlint は機械的な体裁 (一文の長さ・読点・文体・スペース) しか見ない。書き手の不在 (false agency・命題型 H2・体験の壮大化・両論併記・偏愛語・リズムの均一さ) が「AI 臭」である。その検出は別スキル `refine__ai_slop_japanese` が担う。Claude Code には「スキル終了」フックが無いため、この引き継ぎは本フェーズで行う。

Phase 7 のレポートには「AI 臭レビュー引き継ぎ」の判定行を必ず含める。判定を書かずにスキルを終了してはならない。**prose** とは、書き手の主張・体験・主観を含む文書を指す。例はブログ下書き・記事・エッセイ・README の散文部分である。対象が prose の場合、レポート出力の直後に Skill ツールで `refine__ai_slop_japanese` を起動する。

- **引き継がない (スキップする)**: 表・コマンド列・設定値・API リファレンスの羅列が主の文書。書き手の立場が存在せず、AI 臭レビューを常時走らせると過剰になるため。
- 引き継ぐかの判断が曖昧なときは、ユーザーに「AI 臭レビューを続けるか」を尋ねる。
