...
# submit__pull_request skill instead, which generates narrative-style PR
...
jq -n '{
  "decision": "reject",
  "reason": "Direct `gh pr create` is not allowed. Use the submit__pull_request skill instead, which generates a narrative-style PR description (Background, Approach, What Changed, Tradeoffs, Testing, Review Guide) and then monitors CI automatically.\n\nExecute the submit__pull_request skill now to create this PR with a proper narrative description."
}'
