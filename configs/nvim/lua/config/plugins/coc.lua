...
-- <C-TAB> と <S-TAB> キーを使って、補完メニュー（coc#pum#visible()）が表示されている場合は次の候補、前の候補に移動
keyset("i", "<C-TAB>", 'coc#pum#visible() ? coc#pum#next(1) : v:lua.check_back_space() ? "<TAB>" : coc#refresh()', opts)
keyset("i", "<S-TAB>", [[coc#pum#visible() ? coc#pum#prev(1) : "\<C-h>"]], opts)

-- 補完メニューが表示されている場合に <CR>（エンター）キーを押すと選択した補完候補を確定し、補完メニューが表示されていない場合は通常の改行
keyset("i", "<cr>", [[coc#pum#visible() ? coc#pum#confirm() : "\<C-g>u\<CR>\<c-r>=coc#on_enter()\<CR>"]], opts)

-- Use <c-j> to trigger snippets
keyset("i", "<leader>j", "<Plug>(coc-snippets-expand-jump)")
